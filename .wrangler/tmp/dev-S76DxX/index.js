...
// .wrangler/tmp/bundle-Y0gj4U/checked-fetch.js
...
// C:/Users/avezq/AppData/Roaming/npm/node_modules/wrangler/templates/modules-watch-stub.js
...
// .wrangler/tmp/bundle-Y0gj4U/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-Y0gj4U/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
// src/index.ts
var app = createApp();
var supabaseUrl = "https://skotefhdkqtrmykssrxa.supabase.co";
var supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNrb3RlZmhka3F0cm15a3NzcnhhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjM2OTczMTYsImV4cCI6MjAzOTI3MzMxNn0.y0AlptXYIK4pUE4PNBM5tJp2sNWZ_WTtS-i9lNjd4uk";
var supabase = createClient(supabaseUrl, supabaseKey);
var corsMiddleware = (c, next) => {
...
app.get("/categories/:region_id/:device_id", async (c) => {
  const regionId = c.req.param("region_id");
  const deviceId = c.req.param("device_id");
  try {
    const { data: multipliers, error } = await supabase.from("ad_multipliers").select("category_id").eq("region_id", regionId).eq("device_id", deviceId);
    if (error) {
      console.error("Error fetching multipliers:", error);
      return c.json({ error: "Error fetching categories" }, 500);
    }
    const categoryIds = multipliers.map((item) => item.category_id);
    const { data: categories, error: categoriesError } = await supabase.from("categories").select("*").in("id", categoryIds);
    if (categoriesError) {
...
app.get("/devices/:category_id", async (c) => {
  const categoryId = c.req.param("category_id");
  try {
    const { data, error } = await supabase.from("ad_multipliers").select("device_id").eq("category_id", categoryId);
    if (error) {
      console.error("Error fetching devices:", error);
      return c.json({ error: "Error fetching devices" }, 500);
    }
    const deviceIds = data.map((item) => item.device_id);
    const { data: devices, error: devicesError } = await supabase.from("devices").select("*").in("id", deviceIds);
    if (devicesError) {
...
app.post("/calculate-revenue", async (c) => {
  const { region_id, category_id, device_id, mau } = await c.req.json();
  if (!region_id || !category_id || !device_id || !mau) {
    return c.json({ error: "Missing required fields" }, 400);
  }
  try {
    const { data, error } = await supabase.from("ad_multipliers").select("multiplier").eq("region_id", region_id).eq("category_id", category_id).eq("device_id", device_id).single();
    if (error || !data) {
...
app.get("/total-multiplier-by-region", async (c) => {
  try {
    const { data: multipliers, error: multipliersError } = await supabase.from("ad_multipliers").select("region_id, multiplier");
    if (multipliersError) {
    ...
    const { data: regions, error: regionsError } = await supabase.from("regions").select("id, name").in("id", regionIds);
    if (regionsError) {
...
app.get("/categories-by-region", async (c) => {
  try {
    const { data: multipliers, error: multipliersError } = await supabase.from("ad_multipliers").select("region_id, category_id");
    if (multipliersError) {
      console.error("Error fetching multipliers:", multipliersError);
    ...
    const { data: categories, error: categoriesError } = await supabase.from("categories").select("id, name").in("id", categoryIds);
    if (categoriesError) {
      console.error("Error fetching categories:", categoriesError);
      return c.json({ error: "Error fetching categories" }, 500);
    }
    const categoriesByRegion = regions.map((region) => {
      const regionCategories = multipliers.filter((item) => item.region_id === region.id).map((item) => {
...
app.get("/revenue-by-category", async (c) => {
  const { mau, platform } = c.req.query();
  if (!mau || !platform) {
    return c.json({ error: "Missing required query parameters" }, 400);
  }
  try {
    const { data: multipliers, error } = await supabase.from("ad_multipliers").select("region_id, category_id, multiplier").eq("platform", platform);
    if (error) {
      console.error("Error fetching multipliers:", error);
...
app.get("/top-regions-or-categories", async (c) => {
  const { mau, dimension, platform } = c.req.query();
  ...
  try {
    ...
    const topItems = Object.entries(revenueByDimension).sort(([, a], [, b]) => b - a).map(([key, revenue]) => ({ [dimension]: key, revenue }));
...
app.get("/efficient-regions-or-platforms", async (c) => {
  const { revenue, dimension, platform } = c.req.query();
  if (!revenue || !dimension || !platform) {
    return c.json({ error: "Missing required query parameters" }, 400);
  }
  try {
    const { data: multipliers, error } = await supabase.from("ad_multipliers").select("region_id, platform, multiplier").eq("platform", platform);
    if (error) {
      console.error("Error fetching multipliers:", error);
    ...
    const efficientItems = multipliers.map((item) => ({
      ...
      // Adjust if dimension is 'platform'
...
app.get("/revenue-growth", async (c) => {
  const { mau_ranges, dimension, platform } = c.req.query();
...
app.get("/underperforming-regions-or-platforms", async (c) => {
...
// C:/Users/avezq/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
init_checked_fetch();
init_modules_watch_stub();
...
// C:/Users/avezq/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
init_checked_fetch();
init_modules_watch_stub();
...
// C:/Users/avezq/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
init_checked_fetch();
init_modules_watch_stub();
